		...
		while(index < regex.length()){
			...
			if(c == '['){
				...
				while(c != ']'){
					...
					else if(c == '\\'){		//NOTE: have to escape the \ char in Java
						c = regex.charAt(++index);
						acceptedChars.add(c);
					}		
				...
				if(accepted == false){	//We have a ^
					...
					for (int i=0; i<256; i++){	//Loop through all possible chars
			...
			for (int x=1; x<table.length; x++){
				table[x][table.length-1] = 0;
			}
			table[1][table.length-1] = 1;
